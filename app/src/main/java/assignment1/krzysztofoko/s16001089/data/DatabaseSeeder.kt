...
suspend fun seedDatabase(db: AppDatabase) { 
    ...
    // 1. BOOKS SECTION
    val sampleBooks = listOf( // We create a list of all the physical books we want to sell
    ...
    ).map { it.copy(id = it.title.lowercase().replace(" ", "_")) } // This part makes a unique ID for each book using its name
    db.bookDao().insertAll(sampleBooks) // Finally, we push the whole list into the local database table

    // 2. AUDIOBOOKS SECTION
    val sampleAudio = listOf( // Now we do the same for audiobooks
    ...
    ).map { it.copy(id = it.title.lowercase().replace(" ", "_")) } // Again, creating simple IDs like "atomic_habits"
    db.audioBookDao().insertAll(sampleAudio) // Saving all audiobooks to the database

    // 3. COURSES SECTION
    val sampleCourses = listOf( // Here we define the university courses
    ...
    ).map { it.copy(id = it.title.lowercase().replace(" ", "_")) } // Generating unique IDs for courses
    db.courseDao().insertAll(sampleCourses) // Adding courses to the course table

    // 4. GEAR SECTION
    val sampleGear = listOf( // This is for university merch like hoodies and bags
    ...
    ).map { it.copy(id = it.title.lowercase().replace(" ", "_")) } // IDs for gear
    db.gearDao().insertAll(sampleGear) // Pushing merch to the database
    
    // 5. USER SECTION
    val krisId = "user_kris_001" // Defining a fixed ID for the first demo user
    val demoUser = UserLocal( // Creating a local profile for Kris
        id = krisId,
        name = "Krzysztof Oko",
        email = "prokocomp@gmail.com",
        balance = 1000.0, // Giving the student some initial wallet balance
        address = "Wrexham University Campus, Mold Road, LL11 2AW",
        role = "student",
        photoUrl = "file:///android_asset/images/users/avatars/Krzysztof-Oko.jpeg"
    )
    db.userDao().upsertUser(demoUser) // Save or update this user in the database

    val andrejId = "wzYECwgHwTn7Ebhx8vPk7Epuaat2" // This ID matches a real user from our auth system
    val andrejUser = UserLocal( // Creating another local profile for Andrej
        id = andrejId,
        name = "Andrej Nilsen",
    ...
    db.userDao().upsertUser(andrejUser) // Save or update this user as well

    
    // Log that everything is finished and the database is ready!
