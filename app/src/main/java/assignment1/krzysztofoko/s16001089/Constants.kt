package assignment1.krzysztofoko.s16001089

object AppConstants { // Constraints  used in splash screen and about app/developer screen
    const val VERSION_NAME = "0.04.273"// App Still under development is a basic very early version needs more improvements butt very soon will come Alpha version, Can't wait work hard  :D
    const val INSTITUTION = "Wrexham Glyndŵr University"
    const val DEVELOPER_NAME = "Krzysztof Oko"
    const val STUDENT_ID = "S16001089"
    const val PROJECT_INFO = "Assignment 1 - CONL709 Mobile App Development"
    const val APP_NAME = "Glyndŵr Store"
}
